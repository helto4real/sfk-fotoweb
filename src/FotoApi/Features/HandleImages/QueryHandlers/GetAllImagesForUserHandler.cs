...
public class GetAllImagesForUserHandler(PhotoServiceDbContext db, CurrentUser currentUser) : IEmptyRequestHandler<IReadOnlyCollection<ImageResponse>>
{
    private readonly ImagesMapper _mapper = new();

    public async Task<IReadOnlyCollection<ImageResponse>> Handle(CancellationToken cancellationToken)
    {
