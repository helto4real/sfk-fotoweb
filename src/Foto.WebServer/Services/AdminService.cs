...
public class AdminService : IAdminService
{
    ...
    public async Task<UserInfo?> GetUserByUsernameAsync(string username)
    {
        var response =
            await _signInService.RefreshTokenOnExpired(async () =>
                await _httpClient.GetAsync($"api/admin/user/{username}"));
    ...
    public async Task<bool> UpdateUserAsync(UserInfo user)
    {
        var response =
            await _signInService.RefreshTokenOnExpired(async () =>
                await _httpClient.PutAsJsonAsync("api/admin/user", user));
    ...
    public async Task<bool> DeleteUserAsync(string username)
    {
        var response =
            await _signInService.RefreshTokenOnExpired(async () =>
                await _httpClient.DeleteAsync($"api/admin/user/{username}"));
        return response.IsSuccessStatusCode;
    }
    ...
    {
        var response =
            await _signInService.RefreshTokenOnExpired(async () => await _httpClient.GetAsync("api/admin/users"));
    ...
    public async Task<User?> CreateUserAsync(NewUserInfo user)
    {
        var response =
            await _signInService.RefreshTokenOnExpired(async () =>
                await _httpClient.PostAsJsonAsync("api/admin/user", user));
    ...
    {
        var response = await _signInService.RefreshTokenOnExpired(async () =>
            await _httpClient.PostAsJsonAsync("api/admin/users/precreate", new { email }));
